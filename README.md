...
- **单文件分发**：GUI 和 CLI 均打包为单个 `.exe`，下载后直接运行，无需解压。
...
1. 在项目的 [Releases](https://github.com/Chizukuo/NTE-auto-fish/releases) 页面下载 `NTE-Auto-Fish.exe`。
2. 以**管理员权限**直接运行，无需解压，无需安装 Python 环境。
3. （可选）若需精准模板匹配，可在 `.exe` 同目录下创建 `templates/` 文件夹，放入 `button_f.png` 和 `bar_icon_left.png`。若未提供，程序将自动使用分辨率推算。
